...
const ListingsSlider = ({ showAll }) => (
  <section className="listings" id="listings">
    ...
    <Swiper
      ...
      data-aos-delay="200"
    >
      {(showAll ? dummyListings : dummyListings.slice(0, 3)).map((item, idx) => (
        <SwiperSlide key={idx}>
          <div className="listing-card">
            ...
            <div className="listing-details">
              <h3>{item.title}</h3>
              <p>
                {item.price} • {item.beds}bd / {item.baths}ba
              </p>
              <p style={{ fontSize: "0.97em", color: "#7d5b38", marginTop: "0.5em" }}>
                {item.description}
              </p>
            </div>
          </div>
